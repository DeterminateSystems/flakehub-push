      ...
      let
        getFlakeOutputs =
          ...
          rec {
            ...
            defaultSchemas =
              (builtins.getFlake "https://api.flakehub.com/f/pinned/DeterminateSystems/flake-schemas/0.1.5/0190ef2f-61e0-794b-ba14-e82f225e55e6/source.tar.gz?narHash=sha256-G5CxYeJVm4lcEtaO87LKzOsVnWeTcHGKbKxNamNWgOw%3D")
              .schemas;
            ...
            docs = builtins.mapAttrs (outputName: schema: schema.doc or "<no docs>") schemas;
                      ...
                      if attrs ? children then
                        ...
                        // (
                          if inputs.self.includeOutputPaths then
                            {
                              derivation =
                                if attrs ? derivation then builtins.unsafeDiscardStringContext attrs.derivation.drvPath else null;
                              outputs =
                                if attrs ? derivation then
            ...
            inventoryForSystem =
              system:
              inventoryFor (itemSet: !itemSet ? forSystems || builtins.any (x: x == system) itemSet.forSystems);
