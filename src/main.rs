...
async fn execute() -> Result<std::process::ExitCode> {
    ...
    if let Err(e) = response.error_for_status() {
        ...
        if std::env::var("GITHUB_ACTIONS").is_ok() {
            if was_client_error {
                println!("::error title={{FlakeHub: Unauthorized}}::{{Unable to authenticate to FlakeHub. Individuals must register at FlakeHub.com; Organizations must create an organization at FlakeHub.com.}}")
            } else {
                println!("::error title={{FlakeHub: Unauthorized}}::{{Unable to authenticate to FlakeHub. {}}}", e);
            }
        }
        return Err(e.into());
    }
